#include "Match.hpp"
#include "Events.hpp"
#include <iostream>
#include <random>

Match::Match(Team home, Team away)
{
    homeTeam = home;
    awayTeam = away;
}

void Match::printResult()
{
    std::cout << "\n===== Match Result =====\n"
        << homeTeam.name << " " << homeGoals << " : " << awayGoals << " " << awayTeam.name << "\n";
}
//TODO: Verletzungen hinzufügen
void Match::simulate() {
    std::cout << "\n===== Match Ticker =====\n";
    for (int m = 1; m < 91; m++) {
        simulateEvent(m);
    }
    for (int e = 1; e <= extraTime; e++) {
        simulateEvent(90 + e);
    }
}

void Match::simulateEvent(int m) {
    if (chance10(rng) == 1) { // 10% Chance auf irgendein Event
        if (coinFlip(rng) == 1) { //welches team hat ein event
            processEvent(homeTeam, awayTeam, homeGoals, homePlayers, awayPlayers, homeYellowCards, m);
        } else {
            processEvent(awayTeam, homeTeam, awayGoals, awayPlayers, homePlayers, awayYellowCards, m);
        }
    }
}

void Match::processEvent(Team& team, Team& opponent, int& goals, int& players, int opponentPlayers, int& yellowCards, int minute) {
    int eventType = chance10(rng); // Was für ein event? 10%rot, 20% gelb, 70% tor
    if (eventType == 1) { // 1 Rotekarte
        players--;
        extraTime += 2;
        RedCardEvent(minute, team.name);
    }
    else if (eventType < 4) { //2+3 Gelbe Karte
        yellowCards++;
        extraTime++;
        if (yellowCards == 2) {
            players--;
            yellowCards = 0;
        }
        YellowCardEvent(minute, team.name);
    }
    else if (eventType > 3) { // 4-10 Torchance
        if (team.attack * players / 11 > opponent.defense * opponentPlayers / 11 + teamFactor(rng)) {
            goals++;
            GoalEvent(minute, team.name);
        }
    }
}
